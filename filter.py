...
class Filter:
    def __init__(self):
        self.d_taps_upper = None
        self.d_taps_lower = None

    def design_filter(self, samps_per_sym: float, rolloff: float, filter_size: int):
        m = round(filter_size / samps_per_sym)
        power: float = 0
        bb_taps = []
        self.d_taps_lower = list(range(filter_size))
        self.d_taps_upper = list(range(filter_size))
        ...
        for i in tap_index:
            ...
            self.d_taps_lower[filter_size - i - 1] = t1
            self.d_taps_upper[filter_size - i - 1] = t2
